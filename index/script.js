(() => {
  ...
  async function fetchJSON(url, options = {}) {
    const res = await fetch(`${API_BASE}${url}`, { cache: "no-store", credentials: "include", ...options });
    ...
    if (!ct.includes("application/json")) {
      ...
      if (firstChar === "<") {
        throw new Error("Expected JSON but received HTML/XML.");
      }
    ...
    try {
      ...
      console.error("[fetchJSON] JSON parse fail:", e, "Body:", text.slice(0, 120));
      throw e;
    }
  }

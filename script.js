document.addEventListener("DOMContentLoaded", function () {
  ...
  if (isSettings) {
    ...
    if (!form || !account || !container || !sas) {
      console.warn("Settings form elements missing.");
      return;
    }
...
async function loadDashboard() {
  ...
  try {
    ...
    const binTimeMs = binTime ? binTime.getTime() : 0;
    const latestTime = Math.max(whiteTime, binTimeMs);
    ...
    setStatus("bin-status", bin
      ? (timestampsClose(binTimeMs, latestTime) ? "status-green" : "status-yellow")
      : "status-red");
